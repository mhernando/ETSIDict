...
HashMgr::HashMgr(MockFile *tfile)
{
	  tablesize = 0;
  tableptr = NULL;
  int ec = load_tables(tfile);
  if (ec) {
    /* error condition - what should we do here */
    fprintf(stderr,"Hash Manager Error : %d\n",ec);
    fflush(stderr);
...
// mock of load tables with array of n strings (lines of the aff file)
//Miguel Hernando's code
int HashMgr::load_tables(MockFile *rawdict)
{
  int wl, al;
  char * ap;

  // raw dictionary - munched file
  rawdict = fopen(rawdict, "r");
  if (rawdict == NULL) return 1;
