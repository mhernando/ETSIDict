...
//mock
AffixMgr::AffixMgr(MockFile * affpath, HashMgr* ptr) 
{
  // register hash manager and load affix data from aff file
  pHMgr = ptr;
  trystring = NULL;
...
//mock
int  AffixMgr::parse_file(MockFile * afile)
{
  ...
  // open the affix file
  MockFile * afflst;
  afflst = fopen(afile,"r");
  if (!afflst) {
    fprintf(stderr,"Error - could not open affix description file MOCK\n");
    return 1;
  }
...
/* parse in the typical fault correcting table */
int  AffixMgr::parse_reptable(char * line, MockFile * af)
{
   if (numrep != 0) {
      fprintf(stderr,"error: duplicate REP tables used\n");
      return 1;
...
/* MOCK: parse in the character map table */
int  AffixMgr::parse_maptable(char * line, MockFile * af)
{
   if (nummap != 0) {
      fprintf(stderr,"error: duplicate MAP tables used\n");
      return 1;
...
//Mock version
int  AffixMgr::parse_affix(char * line, const char at, MockFile * af)
{
   int numents = 0;      // number of affentry structures to parse
   char achar='\0';      // affix char identifier
   short ff=0;
